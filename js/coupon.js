...
import { deleteCoupon, fetchAllCoupons, handleCouponsStatusChange, storeCoupon } from "../api/couponApi.js";
...
function changeStatus(event)
{
    ...
    handleCouponsStatusChange(
        target.getAttribute('switch_status_id_value'), 
        target.checked,
    )
    .finally(() => {
...
async function fetchCoupons(params) {
    ...
    allCoupons.data.data.forEach((coupon) => {
        ...
        <td>
            <span class="form-check form-switch m-auto">
                ...
                id="flexSwitchCheckChecked" ${coupon.status == true ? 'checked' : ''} switch_status_id_value="${coupon.id}">
            </span>
        </td>
        <td class="tools">
    ...
    Array.from(document.querySelectorAll('[switch_status_id_value]')).forEach((td) => {
        td.addEventListener('click', (e) => changeStatus(e))
    });
}
